...
void CNPC_MetroPolice::StartTask(const Task_t* pTask)
{
	...
	case TASK_METROPOLICE_MOVE_TO_HIGH_COVER: // [MODIFICATION] -  Dynamic Cover
	{
		...
		if (distToTarget <= 50.0f)
		{
			...
			int randomInterval = RandomFloat(8.0f, 12.0f);
			m_flNextPropSearchTime = gpGlobals->curtime + randomInterval;

			Msg("[DEBUG] HIGH_COVER: Arrived! Next prop search in %.1fs\n", randomInterval);
			ClearCondition(TASK_METROPOLICE_MOVE_TO_HIGH_COVER);
			TaskComplete();
			return;
		}
	...
	case TASK_METROPOLICE_MOVE_TO_LOW_COVER: // [MODIFICATION] -  Dynamic Cover
	{
		...
		if (distToTarget <= 50.0f)
		{
			int randomInterval = RandomInt(8.0f, 12.0f);
			m_flNextPropSearchTime = gpGlobals->curtime + randomInterval;
...
AI_BEGIN_CUSTOM_NPC( npc_metropolice, CNPC_MetroPolice )
	...
	DECLARE_TASK(TASK_METROPOLICE_MOVE_TO_LOW_COVER) // [MODIFICATION] -  Dynamic Cover 
	DECLARE_TASK(TASK_METROPOLICE_MOVE_TO_HIGH_COVER)// [MODIFICATION] -  Dynamic Cover 
...
//	DECLARE_CONDITION( COND_METROPOLICE_START_POLICING );
	...
	DECLARE_CONDITION(COND_METROPOLICE_HAS_HIGH_COVER) // [MODIFICATION] -  Dynamic Cover 
	DECLARE_CONDITION(COND_METROPOLICE_HAS_LOW_COVER) // [MODIFICATION] -  Dynamic Cover 
