...
//------------------------------------------------------------------------------
// Constructor. // [MODIFICATION-Dynamic Cover] -  Dynamic Cover
//------------------------------------------------------------------------------
...
void CNPC_Combine::StartTask(const Task_t* pTask)
{
	...
	case TASK_COMBINE_LAUNCH_GRENADE:
	{
		...
		m_bIsReturningGrenade = false;								// Reset the state flag to allow normal grenade interactions again.

		if (m_hPulledObject.Get())									// Check if the NPC is actually holding a grenade.
		{
			IPhysicsObject* pPhysObject = m_hPulledObject->VPhysicsGetObject(); // Get the physics object associated with the grenade entity.
			if (pPhysObject)										// Check if the grenade has a valid physics object.
			{
				// Detach the grenade from the hand and re-enable its physics simulation.
				m_hPulledObject->SetParent(NULL);						// Detach the grenade from the NPC's hand.
				pPhysObject->EnableMotion(true);						// Re-enable the grenade's physics simulation.
				pPhysObject->Wake();								// "Wake up" the physics object so it can be moved.

				// Calculate the launch velocity
				CBaseEntity* pEnemy = GetEnemy();						// Get the NPC's current enemy.
				if (pEnemy)										// Check if a valid enemy exists.
				{
					// --- Throw towards the enemy ---
					Vector vecLaunchDir = pEnemy->WorldSpaceCenter() - m_hPulledObject->WorldSpaceCenter(); // Calculate the direction from the grenade to the enemy.
					VectorNormalize(vecLaunchDir);						// Normalize the direction to get a pure directional vector.

					Vector vecForward, vecUp;
					GetVectors(&vecForward, NULL, &vecUp);				// Get the NPC's own orientation vectors.

					Vector vecVelocity = (vecLaunchDir * 750.0f) + (vecUp * 175.0f); // Calculate the final velocity (towards enemy with a slight upward arc).

					pPhysObject->SetVelocity(&vecVelocity, NULL);			// Apply the calculated velocity to the grenade.
				}
				else
				{
					// --- Fallback: Throw straight ahead if there is no enemy ---
					Vector vecForward, vecUp;
					GetVectors(&vecForward, NULL, &vecUp);				// Get the NPC's own orientation vectors.

					Vector vecVelocity = (vecForward * 750.0f) + (vecUp * 175.0f); // Calculate a velocity straight forward from the NPC.

					pPhysObject->SetVelocity(&vecVelocity, NULL);			// Apply the calculated velocity to the grenade.
				}

				// Cooldown and cleanup
				m_flNextGrenadeCatchTime = gpGlobals->curtime + 2.5f;		// Activate the ability's cooldown to prevent spam.
			}

			m_hPulledObject = NULL;									// Clear the handle, as the NPC is no longer holding the grenade.
		}

		TaskComplete();											// Mark this task as complete.
	...
	{
		m_bForceFastTurn = true;	// Set the 'turbo turn' flag to true.
		DevMsg("StartTask: Starting task to FACE grenade (TURBO ON).\n"); // Log a debug message to the developer console.
		break;				// Exit the switch statement.
	}

	// [MODIFICATION] This task "memorizes" the grenade for the kick schedule.
	// It finds the grenade's source from a danger sound and stores it in a private handle.
	// Without it: The kick logic would not know which object to apply force to.
	case TASK_COMBINE_MEMORIZE_GRENADE:
	{
		CSound* pSound = GetBestSound();	// Get the most prominent sound the NPC can hear.
		if (pSound && pSound->m_hOwner)		// Check if the sound and its owner (the grenade) are valid.
		{
			m_hPulledObject = pSound->m_hOwner; // Store a handle to the grenade entity for later use.
			TaskComplete();			// Mark the task as complete since it's an instantaneous action.
		}
		else
		{
			TaskFail(FAIL_NO_TARGET);	// Fail the entire schedule if no valid grenade could be found.
		}
		break;					// Exit the switch statement.
	}
...
int CNPC_Combine::SelectSchedule(void)
{
	...
	if (m_NPCState != NPC_STATE_SCRIPT)
	{
		...
		if (m_NPCState != NPC_STATE_DEAD && m_NPCState != NPC_STATE_PRONE)
		{
			...
			// Grunts place a high priority on reacting to danger sounds.
			if (HasCondition(COND_HEAR_DANGER))
			{
				// --- GRENADE INTERACTION PRE-CONDITION CHECKS ---
				// 1. Cooldown Verification: Prevents ability re-triggering while on cooldown.
				// If the cooldown is active, the NPC will default to a standard defensive maneuver.
				if (gpGlobals->curtime < m_flNextGrenadeCatchTime)
				{
					 return SCHED_TAKE_COVER_FROM_BEST_SOUND; 
				}

				// 2. State Verification: Disables the ability if the NPC is in the middle of a reload sequence.
				// Self-preservation via evasion is prioritized over the special ability.
				...
				if (pSound)
				{
					...
					{
						// --- ORIGINAL EZ2 DIALOGUE LOGIC ---
						// A dangerous sound is detected; a vocal response may be required.
#ifndef COMBINE_SOLDIER_USES_RESPONSE_SYSTEM
						...
						if (pSoundOwner)
						{
							CBaseGrenade* pGrenadeCheck = dynamic_cast<CBaseGrenade*>(pSoundOwner);
							if (pGrenadeCheck && pGrenadeCheck->GetThrower())
							{
								if (IRelationType(pGrenadeCheck->GetThrower()) != D_LI)
								{
						...
						// --- GRENADE KICK/CATCH ABILITY LOGIC ---
						if (pSound->m_hOwner)
						{
							// 3. Target Validation: Verify the danger sound originates from a standard fragmentation grenade.
							if (strcmp(pSound->m_hOwner->GetClassname(), "npc_grenade_frag") == 0)
							{
								// Defines the effective radius for the special ability.
								const float flAbilityRange = 50.0f;
								float flDist = GetAbsOrigin().DistTo(pSound->m_hOwner->GetAbsOrigin());

								// Check if the grenade is within the effective interaction range.
								if (flDist <= flAbilityRange)
								{
									// 4. Randomized Response: A 50/50 chance determines the action to add behavioral variety.
									if (random->RandomInt(1, 2) == 1)
									{
										// 50% probability of executing the "Kick Grenade" ability.
										return SCHED_COMBINE_RETURN_GRENADE;
									}
									else
									{
										// 50% probability of executing the "Catch and Return" ability.
						...
						// --- END OF GRENADE ABILITY LOGIC ---


						// --- EZ2 FALLBACK LOGIC ---
						// If the grenade ability conditions are not met, execution proceeds to the original EZ2 logic.

						// If the sound is an approaching danger, the NPC has no enemy, and the source is not visible, turn to face it.
						...
						// Default fallback behavior: If no other schedule is selected, take cover from the sound.
						 return SCHED_TAKE_COVER_FROM_BEST_SOUND;  
					}
...
DEFINE_SCHEDULE // [MODIFICATION-Dynamic Cover]
(
SCHED_COMBINE_MOVE_TO_LOW_COVER,
